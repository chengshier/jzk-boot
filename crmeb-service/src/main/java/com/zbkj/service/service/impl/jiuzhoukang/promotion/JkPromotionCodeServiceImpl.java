...
public class JkPromotionCodeServiceImpl implements JkPromotionCodeService {
    ...
    public PageInfo<JkPromotionScene> listScenes(String keyword, Boolean status, PageParamRequest pageParam) {
        ...
        if (StrUtil.isNotBlank(keyword)) query.and(q -> q.like(JkPromotionScene::getSceneCode, keyword)
    ...
    public JkPromotionScene saveScene(JkPromotionSceneSaveRequest request, Long operatorId) {
        ...
        if (request.getSceneTemplate().getBytes(StandardCharsets.UTF_8).length > 32) throw new CrmebException("scene 模板最长32字节");
        ...
        Date now = new Date();
        JkPromotionScene value = request.getId() == null ? new JkPromotionScene().setCreateTime(now).setIsDeleted(false)
    ...
    public JkPromotionCodeResponse generate(Long ownerUserId, String ownerRoleCode, String sceneCode, String requestNo) {
        ...
        JkPromotionCodeCache existing = cacheDao.selectOne(new LambdaQueryWrapper<JkPromotionCodeCache>()
                ...
                .eq(JkPromotionCodeCache::getSceneValue, sceneValue).eq(JkPromotionCodeCache::getIsDeleted, false).last("limit 1"));
        if (existing != null && "SUCCESS".equals(existing.getStatus()) && existing.getFileObjectId() != null) return response(scene, existing);
        JkPromotionCodeCache cache = existing == null ? new JkPromotionCodeCache().setSceneId(scene.getId())
                .setOwnerUserId(ownerUserId).setSceneValue(sceneValue).setRequestNo(requestNo)
                .setStatus("PENDING").setIsDeleted(false).setCreateTime(new Date()) : existing;
        cache.setUpdateTime(new Date()).setErrorMessage(null);
        ...
        try {
            ...
            JkFileObjectResponse file = storageService.store(new ByteArrayMultipartFile(image,
    ...
    private byte[] requestWechatCode(String scene, String page) {
        ...
        body.set("scene", scene); body.set("page", page); body.set("check_path", false); body.set("env_version", "release");
    ...
    private void assertRole(String roles, String role) { if (StrUtil.isBlank(roles)) return; List<String> allowed = Arrays.asList(roles.split(",")); if (!allowed.contains(role)) throw new CrmebException("当前身份不能使用该推广场景"); }
    private String safe(String value) { return value == null ? "未知错误" : value.replace('\r', ' ').replace('\n', ' '); }

    private static class ByteArrayMultipartFile implements MultipartFile {
        private final byte[] bytes; private final String name; private final String contentType;
        private ByteArrayMultipartFile(byte[] bytes, String name, String contentType) { this.bytes = bytes; this.name = name; this.contentType = contentType; }
        ...
        public void transferTo(File dest) throws IOException { FileOutputStream output = new FileOutputStream(dest); output.write(bytes); output.flush(); output.close(); }
    }
}
