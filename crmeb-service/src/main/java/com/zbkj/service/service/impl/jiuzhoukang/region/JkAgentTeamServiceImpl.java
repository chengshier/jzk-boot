...
public class JkAgentTeamServiceImpl implements JkAgentTeamService {
    ...
    public JkPromotionQrcodeResponse promotionQrcode(Long userId) {
        ...
        try { response.setQrCodeBase64(QRCodeUtil.crateQRCode(shareUrl, 480, 480)); }
        catch (Exception e) { throw new IllegalStateException("推广二维码生成失败", e); }
        return response;
    }
    @Override
    public List<JkOptionResponse> eligibleParentOptions(Long userId, String keyword, int limit) {
        ...
        LambdaQueryWrapper<JkUserBusinessRole> roleQuery = new LambdaQueryWrapper<JkUserBusinessRole>()
                ...
                .ne(JkUserBusinessRole::getUserId, userId).orderByDesc(JkUserBusinessRole::getIsPrimary);
        ...
        Map<String, String> roleNames = roleDao.selectList(new LambdaQueryWrapper<JkBusinessRole>().eq(JkBusinessRole::getIsDeleted, false))
                .stream().collect(java.util.stream.Collectors.toMap(JkBusinessRole::getRoleCode, JkBusinessRole::getRoleName, (a,b)->a));
