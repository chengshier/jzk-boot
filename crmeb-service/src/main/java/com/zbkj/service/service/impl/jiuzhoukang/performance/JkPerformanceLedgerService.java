...
public class JkPerformanceLedgerService {
    ...
    /**
     * 兼容旧的“按来源比例冲正”接口。
     * 这里只负责把比例换算为实际冲正金额，真正的冲正、并发控制和负数审计明细统一由正式 Service 完成。
     */
    ...
    public BigDecimal reverseBySource(String sourceType, Long sourceId, BigDecimal ratio, String reason) {
        ...
        List<JkPerformanceRecord> rows = recordDao.selectList(new LambdaQueryWrapper<JkPerformanceRecord>()
        ...
        // 旧接口没有业务 requestNo，只能生成一次性兼容键；关键退款/退货调用方会继续迁移到正式 reverse 接口。
        String requestNo = "PERF_REVERSE_LEGACY:" + sourceType + ":" + sourceId + ":" + IdWorker.getIdStr();
        performanceService.reverse(sourceType, sourceId, null, target, requestNo, reason);
        return target;
    }

    private BigDecimal remaining(JkPerformanceRecord row) {
        return money(row.getPerformanceAmount()).subtract(money(row.getReversedAmount())).max(BigDecimal.ZERO);
    }
