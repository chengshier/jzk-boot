...
-- 九州康根目录：通过名称和 component 查找，不依赖固定 ID。
...
-- V3.1 分组目录。
INSERT INTO eb_system_menu(pid,name,icon,perms,component,menu_type,sort,is_show,is_delete,create_time,update_time)
SELECT @jk_root_id,'身份与关系',NULL,NULL,'/operation/jzk/group/identity','M',100,1,0,@now,@now
WHERE @jk_root_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM eb_system_menu WHERE is_delete=0 AND pid=@jk_root_id AND component='/operation/jzk/group/identity');
INSERT INTO eb_system_menu(pid,name,icon,perms,component,menu_type,sort,is_show,is_delete,create_time,update_time)
...
SET @receive_menu_id = (SELECT id FROM eb_system_menu WHERE is_delete=0 AND component='/operation/jzk/receiveExceptionResolution' ORDER BY id LIMIT 1);
UPDATE eb_system_menu SET pid=@order_group_id,name='异常收货处理',perms='admin:jk:receive:exception:list',component='/operation/jzk/receiveExceptionResolution',is_delete=0,update_time=@now WHERE id=@receive_menu_id;
...
SET @promotion_menu_id = (SELECT id FROM eb_system_menu WHERE is_delete=0 AND (component='/operation/jzk/promotionCode' OR perms='admin:jk:promotion:scene:manage') ORDER BY id LIMIT 1);
UPDATE eb_system_menu SET pid=@operation_tools_group_id,component='/operation/jzk/promotionCode',is_delete=0,update_time=@now WHERE id=@promotion_menu_id;
...
SET @health_report_menu_id = (SELECT id FROM eb_system_menu WHERE is_delete=0 AND (component='/operation/jzk/healthReport' OR perms='admin:jk:health:report:list') ORDER BY id LIMIT 1);
UPDATE eb_system_menu SET pid=@health_group_id,component='/operation/jzk/healthReport',is_delete=0,update_time=@now WHERE id=@health_report_menu_id;
...
SET @subscription_menu_id = (SELECT id FROM eb_system_menu WHERE is_delete=0 AND (component='/operation/jzk/subscriptionTask' OR perms='admin:jk:subscription:task:list') ORDER BY id LIMIT 1);
UPDATE eb_system_menu SET pid=@operation_tools_group_id,component='/operation/jzk/subscriptionTask',is_delete=0,update_time=@now WHERE id=@subscription_menu_id;
INSERT INTO eb_system_menu(pid,name,icon,perms,component,menu_type,sort,is_show,is_delete,create_time,update_time)
