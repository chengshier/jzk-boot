...
SET @now=NOW();
SET @jk_root=(SELECT id FROM eb_system_menu WHERE path='/operation/jzk' AND is_del=0 ORDER BY id LIMIT 1);

DROP PROCEDURE IF EXISTS jk_v31_menu;
DELIMITER $$
CREATE PROCEDURE jk_v31_menu(IN p_name varchar(64),IN p_path varchar(128),IN p_perm varchar(128),IN p_sort int)
BEGIN
  SET @mid=(SELECT id FROM eb_system_menu WHERE path=p_path OR perms=p_perm ORDER BY is_del,id LIMIT 1);
  IF @mid IS NULL THEN
    INSERT INTO eb_system_menu(pid,name,icon,perms,path,menu_type,sort,is_show,is_del,create_time,update_time)
    VALUES(@jk_root,p_name,NULL,p_perm,p_path,'C',p_sort,1,0,@now,@now);
  ELSE
    UPDATE eb_system_menu SET pid=@jk_root,name=p_name,perms=p_perm,path=p_path,menu_type='C',sort=p_sort,is_show=1,is_del=0,update_time=@now WHERE id=@mid;
  END IF;
END$$
DELIMITER ;
...
SET @commission_menu=(SELECT id FROM eb_system_menu WHERE path='/operation/jzk/commissionRule' OR perms='admin:jk:commission:rule:list' ORDER BY is_del,id LIMIT 1);
...
SELECT id,pid,name,perms,path,menu_type FROM eb_system_menu
